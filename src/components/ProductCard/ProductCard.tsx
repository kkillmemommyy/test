...
export const ProductCard = memo(({ thumbnail, title, price, id }: TProps) => {
  ...
  return (
    <div className={cls.card}>
      <img className={cls.img} src={thumbnail} alt='Карточка товара' />
      <div className={cls.description}>
        <p className={cls.title}>{title}</p>
        <p>{`$${price}`}</p>
      </div>
      <button className={cls.addToCart} onClick={addToCartHandle}>
        <svg fill='#000000' version='1.1' id='Capa_1' width='50px' height='50px' viewBox='0 0 902.86 902.86'>
          <g>
            <g>
              <path
